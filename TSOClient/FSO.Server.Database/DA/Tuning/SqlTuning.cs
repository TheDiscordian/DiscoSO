...
namespace FSO.Server.Database.DA.Tuning
{
    public class SqlTuning : AbstractSqlDA, ITuning
    {
        ...
        public void SetTuning(string type, int table, int index, float value)
        {
            var rows = Context.Connection.Execute("UPDATE fso_tuning SET value = @value WHERE tuning_type = @type AND tuning_table = @table AND tuning_index = @index",
                new { type, table, index, value });
            if (rows == 0)
                Context.Connection.Execute("INSERT INTO fso_tuning (tuning_type, tuning_table, tuning_index, value, owner_type) VALUES (@type, @table, @index, @value, 'STATIC')",
                    new { type, table, index, value });
        }
    }
}
