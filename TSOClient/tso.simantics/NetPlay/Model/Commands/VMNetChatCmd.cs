            ...
            if (Message[0] == '/' && Message.Length > 1)
            {
                ...
                switch (cmd.ToLowerInvariant())
                {
                    ...
                    case "tickrate":
                         ...
                         * TSO has 150 ticks per minute (1 minute per 5 irl seconds) */
                        int tick;
                        if (int.TryParse(args, out tick) && tick >= 1) vm.Context.Clock.TicksPerMinute = tick;
                        ...
                        break;
                    case "speed":
                        /* 0 = paused, normal = 1, medium = 3, fast = 10 */
                        int speed;
                        if (int.TryParse(args, out speed) && speed >= 0 && speed <= 999) vm.SpeedMultiplier = speed;
